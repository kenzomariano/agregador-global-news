      ...
      <div className="text-left">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          {item.media_type === "movie" ? (
            <span className="flex items-center gap-1"><Film className="h-3 w-3" /> Filme</span>
          ) : (
            <span className="flex items-center gap-1"><Tv className="h-3 w-3" /> Série</span>
          )}
          {year && <span>{year}</span>}
        </div>
      </div>
